	...
	const char*	path)
{
	...
	if (fd >= 0) {
		...
		if (fd2 < 0) {
			...
			// JAN: TODO: MYSQL 5.7 set_my_errno(errno);
			my_errno=errno;
...
retry:
	if (innobase_commit_concurrency > 0) {
        ...
        trx->mysql_log_offset= static_cast<ib_uint64_t>(pos);
		...
		if (dict_index_is_clust(index)) {
			...
			templ->rec_field_no
				= dict_index_get_nth_col_or_prefix_pos(
					index, v_no, FALSE, true, &prefix_col);
		}
				...
				ROW_SEL_EXACT_PREFIX */
{
	...
	if(!(m_prebuilt->sql_stat_start || (m_prebuilt->trx && m_prebuilt->trx->state == 1))) {
	...
	if (mode1 != PAGE_CUR_UNSUPP && mode2 != PAGE_CUR_UNSUPP) {
		...
		if (dict_index_is_spatial(index)) {
			...
			n_rows = btr_estimate_n_rows_in_range(
				index, range_start, mode1, range_end, mode2, m_prebuilt->trx);
		}
	} else {
	...
	struct st_mysql_value*		value)
{
	...
	if (srv_buf_pool_old_size != srv_buf_pool_size) {
		...
		// JAN: TODO: MySQL 5.7
		//		my_error(ER_BUFPOOL_RESIZE_INPROGRESS,
		//		MYF(0));
