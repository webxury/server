...
void TDC_element::remove_all_unused_tables(TABLE_list *purge_tables,
                                           bool mark_flushed)
{
  TABLE *table;

  mysql_mutex_assert_owner(&LOCK_table_share);
  ...
  */
  if (mark_flushed)
    flushed= true;
  while ((table= free_tables.pop_front()))
  {
...
static my_bool tc_purge_callback(TDC_element *element, tc_purge_arg *arg)
{
  ...
  element->remove_all_unused_tables(&arg->purge_tables, arg->mark_flushed);
                      ...
                      bool kill_delayed_threads)
{
  ...
  element->remove_all_unused_tables(&purge_tables,
