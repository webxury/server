...
Item_num *Item_int::neg(THD *thd)
{
  ...
  */
  ...
  if (value > 0 && name)
    max_length++;                               // Place for '-'
  value= -value;
  // We can't use the given name. name= 0 will force print to generate name
  name= 0;
  return this;
}
...
Item_num *Item_decimal::neg(THD *thd)
{
...
Item_num *Item_uint::neg(THD *thd)
{
  ...
  if (value == LONGLONG_MIN)
    return new (thd->mem_root) Item_int(thd, -value);
...
Item_num *Item_float::neg(THD *thd)
{
  if (value > 0 && presentation)
    max_length++;                               // Reserve place for '-'
  value= -value;
  name= presentation= 0 ;
  return this;
}
